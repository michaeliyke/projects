...
func (user *User) CreateSession() (session U_session, err error) {
...
func Session(w http.ResponseWriter, r *http.Request) (session U_session, err error) {
	cookie, err := GetCookie(r, Config.AuthCookieName)
	if err != nil {
		return
	}
	if err = validation.ValidateUuid(cookie.Value); err != nil {
		return
	}
...
func (user *User) Session() (session U_session, err error) {
	err = Db.QueryRow(
...
func (session *U_session) Check() (valid bool, err error) {
	err = session.Retrieve()
	return err == nil, err
}

// retrieves a user's session from database
func (session *U_session) Retrieve() (err error) {
	err = Db.QueryRow(
		`SELECT s.id, s.uuid, s.keep_login, s.created_at, 
			s.user_id, s.user_uuid,
			u.id, u.name, u.email, u.uuid, u.password, u.privileges
...
func (session *U_session) DeleteByUUID() (err error) {
