...
child = cast(element, child);//no child? use element: use child
   ...
   delta(context, function(context){
    this.each(function(parent){
        if(parent) {
        delta(child, function(child){//a(child)
            parent.insertBefore(child, parent.firstChild);
        });
    }
    });
...
var str = Object.prototype.toString.call(object);
    if (str == "[object String]") {
        ...
        if (boolean == true) {
            object = object.length == +object.length  && str != "[object Comment]" && r != "[object Text]" ? object : [object];
             //If bool is true, make sure we have an array
        }
    }
