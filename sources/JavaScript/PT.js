    ...
    */
      ...
      type: function type(datum, string) {
        if (string && string != null) {
          if (toString.call(string) == "[object String]") {
            ...
            error("pt.type({}, 'object'); //true, type(28); //number", null, ln());
          }
        }
    ...
    (function type_(methods) {
      ...
      primitives: function primitives(datum) {
        ...
        while ((primitive = primitives[counter++])) {
          if (string == primitive) {
            isPrimitive = true;
          }
        } return isPrimitive;
    ...
    return stub = {
      ...
      explode: function explode(anyNoOfArgs) {
        ...
        while (explode.counter > arguments.length) {
          ...
          if (is.list(item)) {
            explode.array = item; explode(explode.array);
          } else { explode.stk.push(item); }
        }
        return explode.stk;
      },
    ...
    */
    ...
    Techie.PT = Techie.pt = Techie.prototype = _techie = {
        ...
        if (!selector) {
          ...
          if (isCollection(selector)) {
            ...
            if (selector.nodeType != +selector.nodeType && selector != window && selector != document && !selector.document) {
              ...
              if (selector.nodeType == 1 || selector == sapi) { //Document, Nodes and Elements
                nodes = [selector];
              } else {
      ...
      removeClass: function removeClass(context, classname) {
        ...
        pt.forEach(function (element, index) {
      ...
      css: function css(element, style, options) {
        ...
        //THREE AGRGUMENTS SUPPLIED
          ...
          if (typeof style !== "string" || typeof (options) !== "string") {
        ...
        //ONE ARGUMENT SUPPLIED
          ...
          if (!style) {
            ret = pt.stringifyAll(this.computedStyle(element), false);
          }
          else {
            if (typeof style === "string") {
              if (style.match(propReg)) {
      ...
      dim: function dim(displayed_item, func_processor) {
        ...
        //dim display
        this.dim = function (displayed_item) {
          this.dim_state = true;
        ...
        this.detatch = function () {
          ...
          var timer = setInterval(function () {
            if (index > 0.15) {
              index -= 0.1; dimmed_item.css = { opacity: index };
            } else {
              clearInterval(timer); dimmed_item.css = ["opacity: 0", "display: none", "visibility: hidden"];
            }
          }, 20);
        };
      },
      ...
      siblings: function siblings(e, fn) {
      ...
      getPreviousSibling: function getPreviousSibling(e) {
        ...
        if (this.prevSib.nodeType != 1) {
          this.prevSib = this.prevSib.previousSibling;
      ...
      last: function last(e, parent) {//.first(div).first("h2").first("<h1></h1>")
        ...
        if (!arguments.length || plain.functions((fn = arguments[0]))) {
          ...
          for (iterator = nodes.length - 1; iterator > 0; iterator--) {
            if ((node = nodes[iterator]) && node.nodeType == 1) {
              lastChild = node; break;
            }
          }
        ...
        if (l(arr)) { //array supplied; work up array items
          return function (array, index) {
            for (; i < array.length; i++) {
              if (array[i].nodeType && !array[i].items) { array[i].items = items; }
    ...
    Techie.PT.DefineAccessors(Techie.prototype, "READY", function gttr() {
      ...
      var timer;
      return function (fn) {
        a(arguments.callee.caller.callee)
