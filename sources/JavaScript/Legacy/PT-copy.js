...
var plain = {
        type: function type(datum, string) {
            var datumType = Object.prototype.toString.call(datum), returnedString = "undefined",
                stringType = Object.prototype.toString.call(string);
            switch(datumType){
                        case "[object String]": returnedString = "string"; break; 
                        case "[object Number]": returnedString = "number"; break; 
                        case "[object Boolean]": returnedString = "boolean"; break; 
                        case "[object Null]": returnedString = "object"; break; 
                        case "[object Undefined]": returnedString = "undefined"; break; 
                        case "[object Nan]": returnedString = "nan"; break; 
                        case "[object Object]": returnedString = "object"; break; 
                        case "[object Function]": returnedString = "function"; break; 
                        case "[object Techie]": returnedString = "techie"; break; 
                        case "[object Array]": returnedString = "object"; break; 
                        default: returnedString = "object";
                    }
            if (string && string != null ) {
                if (stringType == "[object String]") {
                    if (/[\]\[]/.test(string)) { //[object Object] [object Nan]  [object Array]  [object Techie] etc
                        return datumType == string.trim(); //eg type(Object("car"), "[object String]") >>> true
                    }
                   return returnedString == string.trim();
                } else { error( "USAGE: .type({}, 'object'); //true, type(Number(28)); //number", null, ln());
                }
            }
       ...
       isEmptyObject: function isEmptyObject ( obj ) {
        ...
        functions: function functions(value) {
            ...
            return (f && notReg && typeof value.call === "function");
        },
        ...
        numbers: function numbers(x) {
            return x == +x && x % 1 == 0 || toString.call(x) == "[object Number]";
        },
...
function warn (  msg, evaluation, line ) {//error("CSS", "Bad grammer!")
 ...
 function cast( sure, optional ){ //rebase between 2 options
...
var Events = {
    ...
    toggle: function(x, y){ //.toggle(show, hide);
       if (typeof current === "undefined") current = x;
       current == x ? x(): y();
...
return this;
        ...
        addHandler: function(element, type, handler) {
            if (!handler) { //Handle two arguments case
 ...
 prevent: function(event) {
        ...
        evs.forEach(function(name){
            Events[ name ] =  Events[ "on" + capA(name) ] = Events[ "on" + name ] = function(fn, target){
               delta(cast(this, target), function(){ 
                this.each(function(){
                    try{this.addHandler(name, fn.bind(this))}catch(e){/*Do nothing*/}
                });
                });
            }
        });
...
walk_back: function walk ( node, func ){
    func(node);
    node = node.lastChild;
    while( node ) { 
        walk( node, func ); 
...
function matchFunction(fn) {
    return String(fn).match(/[\w\s]+\s*(\s+\w+){0,1}\(([\w\s\W].)+/)[0].trim();
}
...
find: function(selector, context){ //.find(".toogle", [object HTMLDivElement]);
    if (type(selector) !== "string") {return null;}
    if (context) {
        if (!context.nodeType || (context[0] && context[0].nodeType)) {
            throw new Error(".find(selector) called on non HTML object");
        } 
        if (!context.nodeType) {
            context = context[0];
        }
    }
    var ret = (context || sapi).querySelectorAll(selector);
...
Techie.PT.ready( selector );
 return this;
  ...
  context = !context ? document: isList(context) && context[0].querySelectorAll ?
   context[0]: context.querySelectorAll ? context : document;
 ...
 } else if (this.find( selector, is.dom(context) ? context: sapi)) { 
    ...
    } else if (this.find( "#" + selector, selector, is.dom(context) ? context: sapi)) {
       console.warn("Wrong Techie syntax. It ought to be " + "#" + selector);
        return Techie("#"+selector, context);
    }

 } else if ( str != "[object String]" && selector && selector.length == +selector.length ) {
nodes = slice.call(selector);
} else {
    others = slice.call(arguments); length = others.length; this.DefineProperty( this, "length", length, false );
    while ( length ) { this[ index ] = others[ index ]; length--; index++; } index = 0; return this;
}
if (!(nodes[0] && nodes[0].nodeType)) { return this;}
if (!this.length) { //fixes some errors relating to the length propperty of a Techie object
...
pt.DefineProperties(this[index], Techie.PT, false);
    ...
    // prevObject = pt.previous( this );
    // nextObject  = this.next(this)
nextObject = this[0] && this[0].nodeType ? this.next(): null;
...
encrypt: function(plaintext, shiftAmount) {//https://stackoverflow.com/questions/20445039/caesar-cipher-work
    var ciphertext = "";
 ...
 decrypt: function(ciphertext, shiftAmount) { //https://stackoverflow.com/questions/20445039/caesar-cipher-work
    var plaintext = "";
...
createFrag: function createFrag(blob, context) {
        ...
        temp.innerHTML = blob;
         Techie.errors("could not create the node from fragment.", !!temp.firstChild, ln());
...
trace_Node_Origin: function trace(node, func) {
    if(node){
...
matchesSelector: function(selector, context){
    var matches =  Techie.prototype.matches = Techie.prototype.matchesSelector =
    ...
    || Element.prototype.webkitMatchesSelector || function matchesSelector( selector, context ) {
    var matched, found = false;
    if(!context){context = is.dom(this) ? this: sapi};
        if (!is.dom(context)) {
            throw new Error("Context should be empty or a HTMLElement object. @ "+ matchFunction(arguments.callee.caller));
        }
        context = is.collection(context) ? context[0]: context
            var matched = context.querySelectorAll ( selector );
            return matched[0] && matched[0].nodeType && (matched[0].nodeType  == +matched[0].nodeType);
        };
}(),
