...
if (object.length) {  length = object.length;
    for (; index < length; index++) {
        if (action && action.call &&  action.call(This || Techie().push(object[index]), object[index], index, object) === false ) {  break; }
    } } else {  for (index in object) {
        if ( action && action.call && object.hasOwnProperty( index )) {
            if (action.call( This || Techie().push(object[index]), object[index], index,  object ) === false) { break; } 
} } }
return object;
},
    ...
    once: function once (fn, This){
        ...
        fn.call(  This || Techie().push(this[ jota ]), this[ jota ], jota, this );
        break;
        }
        return this;
    },
...
if (object.length) {  length = object.length;
    for (; index < length; index++) {
        if ( action && action.call(This || Techie().push(object[index]), object[index], index, object) === false ) {  
            break; 
        } else{ /*Fix stuff here*/ }
    }
     } else {  for (index in object) {
...
createFrag: function createFrag(blob) {
...
dissolve(this.type(selector, "string") ? sapi.querySelectorAll(selector):(is.list(selector) ? selector:[selector]), this);
