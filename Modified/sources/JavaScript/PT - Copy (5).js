...
var is = {
   ...
   functions: function isFunction(x){
    ...
    dom: function isDom(d){
        return !!(is.html(d) || is.collection(d) || d == document);
    },
...
// I want to make a connection between dom, evernt, and handler
  var EventsHook = { 
    hook: function(dom, event, handler){ 
      this[handler] = [dom, event];
    ...
    empty: function(click){
      ...
      click = this[click]; var Handler;
      click.forEach(function(hanlder){
        Handler = this[handler];        
      Techie.removeHandler(Handler[0], Handler[1], handler);
      }.call(this));
      this[click] = null;
      return this;
    },
                ...
                this.each(function(element){
                    element.addEventListener ? element.addEventListener(type, handler, false) :
                        element.attachEvent ? element.attachEvent( "on" + type, handler) :
                            ...
                            EventsHook.hook(element, type, handler);
                });
            });
          return this;
        },
    ...
    forEach: function forEach ( action, object, This )  {
        ...
        return object;
            ...
            once: function once (fn, This){
                ...
                fn.call( this[ jota ] || This, this[ jota ], jota, this );
                break;
                }
                return this;
            },
...
alpha = this.alpha, capA = this.capA, run = run_the_DOM = this.run_the_DOM,
   pt = PT = Techie, Log, stringify, stringifyAll,
        Store = {},
        search = ("str").search,
...
Techie: function PT(selector, context) {
 ...
 var cntxt = context, data;
 this.ready(function( $, body, head, doc, _, w, Log, stringify, stringifyAll, a ){
this.parameters = this.defaultParameters = this.parameters.splice(1, 0, "element");
    data  = $(selector);
    return cntxt.apply(data, [$, data[0], body, head, doc, _, w, Log, stringify, stringifyAll, a]);
}); 
}
...
tostring = function toString( arg, nothing ){
...
Techie.PT.DefineProperties(this, {
    isTechie: true, techieString: "[object Techie]", techie: "[object Techie]", toString: tostring
}, false);
...
Techie.PT.ready( selector );
    ...
    if(isCollection(selector)){
     // "[object Nodelist]" "[object HTMLCollection]" "[object Techie]"
        ...
        if (selector.nodeType != +selector.nodeType && selector != window && selector != document && !selector.document) {
            this.each.call(selector, function(property, index){
 ...
 } else if ( str != "[object String]" && (selector.length == +selector.length) ) {
nodes = slice.call(selector);
} else {
    ...
    delta(cast(this, is.html(element)? element: null), function(){
        this.each(function(element){
            classes.forEach(function(str){ 
                if (is.html(element)) {
                    element.classList ? element.classList.add(str): element.className += " " + this.trim();
                }
            });
        });
    });
...
var str = x, collector = [], func, length = arguments.length, prop,val;
...
'border': '1px solid olive','margin': 'inherit','font-size': '10px'
        ...
        Log: function Log(text, bool, element) {
            ...
            Techie.PT.ready(function(body, head, doc, PT, _, w) {
                ...
                var log = this.createFrag(  "<h3>" + text + "</h3> <hr />" );
                log.id = "err";
                if (!is.dom(element)) {element = body;}
                 Techie(element).prependChild( "<div></div>" ).first().first(log).css({
...
'word-wrap': 'break-word', padding: "4px 6px", borderRadius: "2px"
        ...
        stringifyAll: function stringifyAll(object, bool, element) {
          ...
          Techie.PT.Log(store) ;return stk;
        },
        ...
        stringify: function stringify(obj, element) {
            ...
            return  is.boolean(element) ? store: delta(element, function(){
              is.dom(this) ?  this.each(function(dom){ 
                     this.html(store); //log result in all matching element nodes
                }): Techie.PT.Log(store); //Log result to the body
